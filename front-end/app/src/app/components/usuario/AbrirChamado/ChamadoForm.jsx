...
export default function ChamadoForm({
  ...
  return (
    <form onSubmit={handleSubmit} className="flex flex-col gap-8" noValidate>
      ...
      <label className="flex flex-col text-gray-800 font-semibold relative">
        <span className="mb-2">Anexar Imagem</span>
        <input
          type="file"
          accept="image/*"
          onChange={(e) => setImagem(e.target.files[0])}
          className={`mt-1 p-2 border rounded-lg focus:outline-none focus:ring-4 focus:ring-red-400 transition shadow-sm ${erros.imagem ? 'border-red-500 ring-red-400' : 'border-gray-300'}`}
          aria-invalid={!!erros.imagem}
          aria-describedby="error-imagem"
        />
        {erros.imagem && <p id="error-imagem" className="text-red-600 mt-1 text-sm font-medium absolute bottom-[-1.5rem] left-0">{erros.imagem}</p>}
      </label>
